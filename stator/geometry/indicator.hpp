...
namespace stator {
  namespace geometry {
    ...
    { return store(pow<2>(try_simplify(deltarij + bi.center() - bj.center())) - pow<2>(bi.radius())); }
    ...
    { return store(indicator(bj, bi, -deltarij)); }
    ...
    { return store(pow<2>(try_simplify(deltarij + bi.center() - bj.center())) - pow<2>(bi.radius() + bj.radius())); }
    ...
    { return store(try_simplify(dot(bj.normal(), deltarij + bi.center() - bj.center()) - bi.radius())); }
    ...
    { return store(indicator(b1, b2, Null()) < 0); }
