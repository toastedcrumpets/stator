...
#define IS_CONSTANT(f, letter) std::is_same<decltype(store(derivative(f, Variable<letter>()))), Null>::value
    ...
    */
    ...
    auto integrate(const T& a, Variable<letter>)
      -> typename std::enable_if<IS_CONSTANT(a, letter), decltype(store(a * Variable<letter>()))>::type
    ...
    auto integrate(const MultiplyOp<LHS, RHS>& a, Variable<Letter> x)
      -> typename std::enable_if<IS_CONSTANT(a._l, Letter), decltype(store(a._l * integrate(a._r, x)))>::type
    { return a._l * integrate(a._r, x); }

    /*! \brief distribute integration through RHS constant multiplication. */
    template<char Letter, class LHS, class RHS>
    auto integrate(const MultiplyOp<LHS, RHS>& a, Variable<Letter> x)
      -> typename std::enable_if<IS_CONSTANT(a._r, Letter), decltype(store(integrate(a._l, x) * a._r))>::type
