...
namespace sym
{
  ...
  std::string repr(const sym::RTBase &);
  template <class Config = DefaultReprConfig>
  std::string repr(const sym::Expr &);
  template <class Op>
  ...
  */
  struct Expr : public shared_ptr<const RTBase>, public SymbolicOperator<Expr>
  {
    ...
    inline Expr(const detail::NoIdentity &) { stator_throw() << "This should never be called as NoIdentity is not a usable type"; }

    template <class... Args>
    explicit Expr(const Array<Args...> &);
    ...
    template <class RHS>
    Expr operator[](const RHS &rhs) const;
...
namespace sym
{
  ...
  namespace detail
  {
    ...
    struct ComparisonVisitor : public sym::detail::VisitorHelper<ComparisonVisitor<LHS>, bool>
    {
      ...
      template <class RHS>
      auto apply(const RHS &r)
      {
        return compare_impl(r, detail::select_overload{});
      }

      template <class RHS>
      auto compare_impl(const RHS &r, detail::choice<0>) -> decltype(_l == r)
      {
        return _l == r;
      }

      template <class RHS>
      bool compare_impl(const RHS &, detail::choice<1>)
      {
        return false;
      }
    };
  ...
  */
  ...
  namespace detail
  {
    ...
    {
      template <class T>
      std::pair<int, int> apply(const T &rhs)
      {
        return BP(rhs);
      }
    };
...
namespace sym
{
  ...
  namespace detail
  {
    ...
    {
      DoubleDispatch1(const Expr &RHS, Visitor &visitor) : _RHS(RHS), _visitor(visitor) {}
    ...
    struct SimplifyRT : VisitorHelper<SimplifyRT>
    {
      ...
      */
      ...
      {
        //We try applying the operator, there are specialised overloads of the operators that might do something
      ...
      struct UnaryEval : VisitorHelper<UnaryEval<Op>>
      {
        ...
        template <class T>
        Expr apply(const T &arg) { return Expr(); }
      };
  ...
  namespace detail
  {
    ...
    {
      SubstituteDictRT(const DictRT &replacement) : _replacement(replacement)
      {
      }
  ...
  RetType RTBase::visit(detail::VisitorInterface<RetType> &c) const
  {
    switch (_type_idx)
    {
    case detail::Type_index<ConstantRT<double>>::value:
      return c.visit(static_cast<const ConstantRT<double> &>(*this).get());
    case detail::Type_index<VarRT>::value:
      return c.visit(static_cast<const VarRT &>(*this));
    case detail::Type_index<UnaryOp<Expr, detail::Sine>>::value:
      return c.visit(static_cast<const UnaryOp<Expr, detail::Sine> &>(*this));
    case detail::Type_index<UnaryOp<Expr, detail::Cosine>>::value:
      return c.visit(static_cast<const UnaryOp<Expr, detail::Cosine> &>(*this));
    case detail::Type_index<UnaryOp<Expr, detail::Log>>::value:
      return c.visit(static_cast<const UnaryOp<Expr, detail::Log> &>(*this));
    case detail::Type_index<UnaryOp<Expr, detail::Exp>>::value:
      return c.visit(static_cast<const UnaryOp<Expr, detail::Exp> &>(*this));
    case detail::Type_index<UnaryOp<Expr, detail::Absolute>>::value:
      return c.visit(static_cast<const UnaryOp<Expr, detail::Absolute> &>(*this));
    case detail::Type_index<UnaryOp<Expr, detail::Arbsign>>::value:
      return c.visit(static_cast<const UnaryOp<Expr, detail::Arbsign> &>(*this));
    case detail::Type_index<BinaryOp<Expr, detail::Add, Expr>>::value:
      return c.visit(static_cast<const BinaryOp<Expr, detail::Add, Expr> &>(*this));
    case detail::Type_index<BinaryOp<Expr, detail::Subtract, Expr>>::value:
      return c.visit(static_cast<const BinaryOp<Expr, detail::Subtract, Expr> &>(*this));
    case detail::Type_index<BinaryOp<Expr, detail::Multiply, Expr>>::value:
      return c.visit(static_cast<const BinaryOp<Expr, detail::Multiply, Expr> &>(*this));
    case detail::Type_index<BinaryOp<Expr, detail::Divide, Expr>>::value:
      return c.visit(static_cast<const BinaryOp<Expr, detail::Divide, Expr> &>(*this));
    case detail::Type_index<BinaryOp<Expr, detail::Power, Expr>>::value:
      return c.visit(static_cast<const BinaryOp<Expr, detail::Power, Expr> &>(*this));
    case detail::Type_index<BinaryOp<Expr, detail::Equality, Expr>>::value:
      return c.visit(static_cast<const BinaryOp<Expr, detail::Equality, Expr> &>(*this));
    case detail::Type_index<BinaryOp<Expr, detail::ArrayAccess, Expr>>::value:
      return c.visit(static_cast<const BinaryOp<Expr, detail::ArrayAccess, Expr> &>(*this));
    case detail::Type_index<BinaryOp<Expr, detail::Units, Expr>>::value:
      return c.visit(static_cast<const BinaryOp<Expr, detail::Units, Expr> &>(*this));
    case detail::Type_index<BinaryOp<Expr, detail::Uncertainty, Expr>>::value:
      return c.visit(static_cast<const BinaryOp<Expr, detail::Uncertainty, Expr> &>(*this));
    case detail::Type_index<ArrayRT>::value:
      return c.visit(static_cast<const ArrayRT &>(*this));
    case detail::Type_index<DictRT>::value:
      return c.visit(static_cast<const DictRT &>(*this));
    case detail::Type_index<UnaryOp<Expr, detail::Negate>>::value:
      return c.visit(static_cast<const UnaryOp<Expr, detail::Negate> &>(*this));
    default:
      stator_throw() << "Unhandled type index (" << _type_idx << ") for the visitor";
    }
  }
